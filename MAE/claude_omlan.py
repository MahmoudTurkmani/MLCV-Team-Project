...
class BirbSet(Dataset):
    def __init__(self, df, root, clip_length, label_to_idx, is_train=False,
        ...
        # If a Bird-MAE feature_extractor is provided, __getitem__ runs it
        # HERE (inside the Dataset, i.e. inside a DataLoader worker process)
        # instead of leaving it for BirdMAEClassifier.forward() to do on the
        # main process. That lets num_workers>1 parallelize the (CPU-bound)
        # mel extraction across processes and overlap it with the GPU doing
        # the previous batch's forward/backward -- same trick omlan uses.
        # Only leave this as None when waveform-domain batch mixing (MixCut)
        # is active, since that has to happen post-collate, before extraction.
        self.feature_extractor = feature_extractor

        # single_random_crop=True: __len__ is one entry per source file (not
        # one entry per 5s window), and __getitem__ picks a random window
        # from that file each call -- exactly what omlan's BirdMAEDataset
        # does. This is what actually controls how many samples one epoch
        # contains; exhaustively enumerating every non-overlapping window in
        # every file (the default below) inflates epoch size several-fold
        # for anything longer than ~15-20s, which is most of BirdCLEF's
        # Xeno-canto recordings. Turn it off if you specifically want full
        # sliding-window coverage of long recordings and can afford longer
        # epochs.
        self.single_random_crop = single_random_crop
        ...
        # Group every enumerated window by its source file so
        # single_random_crop mode can pick ONE random window per file per
        # __getitem__ call instead of iterating every window every epoch.
        if self.single_random_crop:
            self._file_to_window_idxs = {}
            for i, path in enumerate(self.clips):
                self._file_to_window_idxs.setdefault(path, []).append(i)
            self._epoch_files = list(self._file_to_window_idxs.keys())

    def __len__(self):
        if self.single_random_crop:
            return len(self._epoch_files)
    ...
    def __getitem__(self, idx):
        if self.single_random_crop:
            # idx indexes files, not windows -- pick one of that file's
            # precomputed 5s windows at random this call.
            file_path = self._epoch_files[idx]
            window_idxs = self._file_to_window_idxs[file_path]
            idx = window_idxs[np.random.randint(len(window_idxs))] if self.is_train \
                else window_idxs[len(window_idxs) // 2]  # deterministic for val
        ...
        try:
            ...
            if self.raw_waveform_output:
                ...
                if self.feature_extractor is not None:
                    # Do the (CPU-bound) mel extraction HERE, inside this
                    # DataLoader worker process, in parallel with every other
                    # worker and overlapped with the GPU training the previous
                    # batch. This replaces the old design of doing it serially,
                    # one sample at a time, inside BirdMAEClassifier.forward()
                    # on the main process -- that fully blocked the GPU on CPU
                    # work every single step. Returns the same tensor shape
                    # the model's own _extract_spec() would have produced, so
                    # BirdMAEClassifier.forward() can skip extraction entirely
                    # when it receives already-processed input.
                    wf_np = waveform.squeeze(0).numpy().astype(np.float32)
                    feat  = self.feature_extractor(wf_np, return_tensors="pt")
                    key   = "input_values" if "input_values" in feat else list(feat.keys())[0]
                    spec  = feat[key].squeeze(0)
                    return spec, target
...
class BirdMAEClassifier(nn.Module):
    ...
    published example only shows one clip at a time, not a batch, so it's
    still called one sample at a time. The difference from the original
    version of this file is WHERE that loop runs: normally (expects_
    precomputed_features=True) it's run once per sample inside BirbSet's
    __getitem__, i.e. inside parallel DataLoader worker processes, and
    forward() just receives an already-batched tensor. It only falls back to
    looping here, serially, on the main process, when MixCut needs to mix
    raw waveforms after collation (extraction has to happen after mixing).

    NOTE: Bird-MAE-Huge is a large ViT-scale model -- several times slower
    per forward/backward pass than Base regardless of any of the above.
    Consider Bird-MAE-Base for iteration speed, or precomputing/caching
    embeddings once (since the frozen backbone's output for a given raw clip
    never changes across epochs).
    """
    ...
    def __init__(self, model_name="DBD-research-group/Bird-MAE-Huge",
                 ...
                 hidden_dim=512, dropout=0.3, feature_extractor=None,
                 expects_precomputed_features=False):
        super().__init__()
        self.sample_rate = sample_rate
        self.freeze_backbone = freeze_backbone

        # expects_precomputed_features=True means the DataLoader (via
        # BirbSet's own feature_extractor) already turned each waveform into
        # a mel spectrogram inside worker processes -- forward() then just
        # consumes the batched tensor directly instead of looping per-sample
        # over self.feature_extractor on the main process. Only leave this
        # False when waveform-domain MixCut is active, since mixing has to
        # happen post-collate, before extraction, and so can't be precomputed
        # per-sample in the Dataset.
        self.expects_precomputed_features = expects_precomputed_features

        # Reuse a feature_extractor built outside (e.g. shared with the
        # Dataset) instead of re-downloading/re-instantiating it here.
        self.feature_extractor = feature_extractor or AutoFeatureExtractor.from_pretrained(
    ...
    def forward(self, batch):
        if self.expects_precomputed_features:
            # batch is already a stacked mel-spectrogram tensor produced by
            # BirbSet's worker processes -- no CPU work left to do here.
            specs = batch
        else:
            # waveform_batch: tensor [B, 1, T], can be on CPU or CUDA.
            # Slow path: only hit when MixCut needs to mix raw waveforms
            # post-collate, so extraction can't be precomputed per-sample.
            device = batch.device
            specs = []
            for wf in batch:
...
if __name__ == "__main__":
    ...
    if BIRDMAE_MODEL_SIZE == "Huge":
        print("⚠️  --backbone_size Huge is a ViT-Huge-scale model -- several times "
              "slower per forward/backward pass than Base, on top of everything "
              "else. Use Base for fast iteration; switch to Huge only for a final run.")

    # Build ONE feature_extractor and share it between the Dataset (which
    # runs it inside parallel worker processes) and the model (which only
    # needs its own copy as a fallback for the MixCut path below).
    feature_extractor = AutoFeatureExtractor.from_pretrained(
        BIRDMAE_MODEL_NAME, trust_remote_code=True
    )
    # MixCut mixes raw waveforms together AFTER collation (see
    # apply_mixcut_waveform), so extraction can't happen per-sample inside
    # the Dataset when it's on -- it has to happen after mixing, in
    # forward(), same as before. Everywhere else, precompute in the Dataset
    # workers so extraction runs in parallel with, instead of blocking, the
    # GPU. This is the main reason this script used to take ~28x longer per
    # epoch than omlan.
    PRECOMPUTE_FEATURES = not USE_MIXCUT
    if not PRECOMPUTE_FEATURES:
        print("ℹ️  MixCut is on: feature extraction stays on the per-batch "
              "critical path (can't be precomputed per-sample). Expect this "
              "run to be noticeably slower than a MixCut-off run.")
    ...
    dset_train = BirbSet(
        ...
        feature_extractor=feature_extractor if PRECOMPUTE_FEATURES else None,
        single_random_crop=True,   # one random 5s crop per file per epoch, like omlan --
                                     # keeps epoch size proportional to file COUNT rather
                                     # than total audio duration. Set False to instead
                                     # exhaustively cover every window of every file
                                     # (bigger, slower epochs; more coverage of long clips).
    )
    ...
    #
    # IMPORTANT: with single_random_crop=True, len(dset_train) == number of
    # FILES (one entry per source file), not number of windows -- the sampler
    # must draw indices in that same space, or DataLoader will hand it
    # out-of-range indices. Use each file's first window's label as that
    # file's representative label for weighting purposes.
    if dset_train.single_random_crop:
        sampler_labels = [
            dset_train.labels[dset_train._file_to_window_idxs[f][0]]
            for f in dset_train._epoch_files
        ]
    else:
        sampler_labels = dset_train.labels

    clip_label_counts = Counter(sampler_labels)
    sample_weights = torch.tensor(
        [1.0 / clip_label_counts[lbl] for lbl in sampler_labels],
        dtype=torch.float32
    )
    ...
    dset_val = BirbSet(
        ...
        feature_extractor=feature_extractor if PRECOMPUTE_FEATURES else None,
        single_random_crop=True,   # deterministic middle-window pick when not training
    )
    ...
    model = BirdMAEClassifier(
        ...
        expects_precomputed_features=PRECOMPUTE_FEATURES,
    ).to(device)
